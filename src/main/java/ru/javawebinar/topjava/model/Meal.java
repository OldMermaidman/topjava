...
@NamedQueries({
        ...
        @NamedQuery(name = Meal.BETWEEN_HALF_OPEN, query = "SELECT m FROM Meal m WHERE m.user.id=:userId  AND m.dateTime >= :start_date_time AND m.dateTime < :end_date_time ORDER BY m.dateTime DESC"),
